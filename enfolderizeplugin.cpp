...
K_PLUGIN_FACTORY(EnfolderizePluginFactory, registerPlugin<EnfolderizePlugin>();)
K_EXPORT_PLUGIN(EnfolderizePluginFactory("enfolderizeplugin"))

EnfolderizePlugin::EnfolderizePlugin(QObject* parent, const QList<QVariant>& args) :
...
EnfolderizePlugin::~EnfolderizePlugin()
{
    GitWrapper::freeInstance();
}

QString EnfolderizePlugin::fileName() const
{
    return QLatin1String(".git");
}

int EnfolderizePlugin::readUntilZeroChar(QIODevice* device, char* buffer, const int maxChars) {
...
bool EnfolderizePlugin::beginRetrieval(const QString& directory)
{
...
void EnfolderizePlugin::endRetrieval()
{
}

KVersionControlPlugin2::ItemVersion EnfolderizePlugin::itemVersion(const KFileItem& item) const
{
...
QList<QAction*> EnfolderizePlugin::actions(const KFileItemList &items) const
{
...
QList<QAction*> EnfolderizePlugin::contextMenuFilesActions(const KFileItemList& items) const
{
...
QList<QAction*> EnfolderizePlugin::contextMenuDirectoryActions(const QString& directory) const
{
    QList<QAction*> actions;
    if (!m_pendingOperation){
...
void EnfolderizePlugin::addFiles()
{
...
void EnfolderizePlugin::removeFiles()
{
...
void EnfolderizePlugin::checkout()
{
...
void EnfolderizePlugin::createTag()
{
   TagDialog dialog;
    if (dialog.exec() == QDialog::Accepted) {
...
void EnfolderizePlugin::slotOperationCompleted(int exitCode, QProcess::ExitStatus exitStatus)
{
    m_pendingOperation = false;
...
void EnfolderizePlugin::slotOperationError()
{
...
QString EnfolderizePlugin::parsePushOutput()
{
...
QString EnfolderizePlugin::parsePullOutput()
{
    char buffer[256];
    while (m_process.readLine(buffer, sizeof(buffer)) > 0) {
        const QString line(buffer);
...
void EnfolderizePlugin::execGitCommand(const QString& gitCommand,
...
void EnfolderizePlugin::startGitCommandProcess()
{
